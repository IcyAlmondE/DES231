public class BST<T extends Comparable<T>> extends BT<T> {
    ...
    public void insert(T newdata) {
        ...
        if (root == null) {
            ...
            current = new BTNode<>(newdata);
            if(newdata.compareTo(parent.element)<0) parent.left = current;
    ...
    public T delete(T item) {
        ...
        while (current != null) {
            // insert your code
            if(item.compareTo(current.element)==0) break;
            parent = current;
    ...
    public boolean search(T data) {
        ...
        BTNode<T> current = root;
        while(current != null){
            if(data.compareTo(current.element)==0) return true;
            else if(data.compareTo(current.element)<0) current = current.left;
    ...
    private T findClosestValue(BTNode<T> node, T target, T closest) {
        ...
        if (Math.abs(nodeVal - targetVal) < Math.abs(closestVal - targetVal)) {
        ...
        if (target.compareTo(closest)<0/* condition to traverse left subtree*/) {
            return findClosestValue(node.left, target, closest)/* recursive call for left subtree */;
        } else if (target.compareTo(closest)>0/*condition to traverse right subtree*/) {
            return findClosestValue(node.right, target, closest)/* recursive call for right subtree */;
        } else {
            return node.element;
        }
    }
