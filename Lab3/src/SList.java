...
public class SList<T> {
    ...
    void addAtIndex(int index, T element) {
        ...
        if(index==0) addFirst(element);
            ...
            SNode<T> current, newNode = new SNode<T>(element);
            current = first;
    ...
    T removeLast() {
        ...
        if(size==0) return null;
            ...
            SNode<T> temp = last, current = first;
    ...
    void reverse() {
        ...
        while (current!=null) { // Complete the condition
            next = current.next;
            current.next = previous/* update next pointer */;
        ...
        for(int i=0; i<size-1; i++){
            last = temp;
            if(last.next!=null) last = temp.next;
            else break;
        }
//        last = /* assign last */;
        first = previous;
